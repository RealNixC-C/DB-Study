...
-- 정답: 
/*
SMALLINT는 2바이트로 메모리를 절약하면서 65,535까지의 정수를 저장할 수 있기 때문이다.
*/
...
-- 정답: 
/*
CHAR() = 고정형 문자열, 50개 미만의 값을 저장, 공백포함 고정으로 50자리 출력
VARCHAR() = 가변형 문자열, 50개 미만의 값을 저장, 공백 제외하고 출력
*/
...
/*
DATE = YYYY/MM/DD 날짜만
DATETIME = YYYY/MM/DD HH:MM:SS 날짜와 시간
*/
...
-- 정답: 
-- ENUM에 제한된 값 외에 다른값이 저장되지 못함
-- 성별은 2가지밖에 없기때문에 잘못된 입력을 방지할 수 있음
...
-- 정답: 
-- 주로 용량이 큰 이미지, 오디오 파일이나 동영상들을 저장하는데 적합
...
/*
정밀도의 차이
FLOAT 소수점 이하 약 7자리표현 4Byte
DOUBLE 소수점 이하 약 15~17자리표현 8Byte
*/
...
-- 정답: 
-- 문자 '박'을 포함한 다음 문자2개 (MINE)
-- 고객 이름이 '박'으로 시작하는 3글자를 찾는다
...
-- 정답: 
-- order_time의 시간부분을 추출한 값이 9~17인지 조회
...
SELECT title, start_time
FROM events
WHERE (title LIKE '%컨퍼런스%' OR title LIKE '%컨트리뷰션%') AND TIME(start_time) >= 11;
