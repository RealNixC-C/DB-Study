-- Active: 1749607595528@@127.0.0.1@3306@quiz
-- 1. 서브쿼리 확인 문제
...
SELECT 
	name AS '직원 이름', 
	COUNT(*) AS '참여중인 프로젝트 수'
FROM employees e
JOIN employee_projects ep ON e.id = ep.employee_id
GROUP BY e.id, name; -- 직원별로 그룹화(중복 이름이 있을 수도 있으니 id까지 그룹으로 묶어서)
-- INNER JOIN을 쓰면 프로젝트에 참여한 직원만 나옴
-- LEFT JOIN을 사용하는 이유는 프로젝트에 참여하지 않은 직원도 0건으로 표시하기위해
-- 여기서 또 주의할점은 COUNT(*)을 사용할 경우 Null값이 들어있는 행도 카운트함
-- 명확하게 하기 위해서는 project_id 를 명시적으로 카운트할것 COUNT(project_id)
...
SELECT 
	name AS 부서명,
	employees_number AS '직원 수'
FROM (
	SELECT d.name, COUNT(*) AS employees_number
	FROM departments d
	JOIN employees e ON d.id = e.department_id 
	GROUP BY d.name) AS sub
...
GROUP BY d.name
HAVING AVG(salary) >= (
	-- SELECT SUM(salary) / COUNT(*) -- AVG(salary) 평균 조회할때 전체합 / 카운트수로 하는 버릇있음
	SELECT AVG(salary)
  FROM employees)
