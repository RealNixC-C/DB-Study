...
-- ② 도서 테이블의 재고 칼럼에 UNSIGNED 제약 조건을 추가한다. -- 물리적
-- ③ 대출 기록 테이블의 외래키 관계를 명시한다. -- 논리적
-- ④ 테이블의 인덱스 설계를 통해 쿼리 성능을 최적화한다. -- 물리적

-- 정답: 1 (속성 정의는 개념적 모델링에서 하고, 논리적 모델링에서도 함)
...
-- 정답: 2
-- 각 속성은 이미 원자값(더 이상 분해할 수 없는 값)을 가지고, 
-- 같은 속성끼리 동일한 자료형을 가지,
