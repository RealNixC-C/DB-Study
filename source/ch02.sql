-- Active: 1749607595528@@127.0.0.1@3306@mapdonalds
-- 주석
...
-- (MySQL 서버 운영 관리에 중요한 역할, 우리가 건드릴 일 없음)

-- 데이터베이스 생성 (왼쪽하단 Schemas탭으로 변경 후 확인)
...
CREATE TABLE burgers (
    ...
    -- VARCHAR(N): 문자를 최대 N자리까지 저장, 문법상 필수(가변 길이 타입이면 저장할 수 있는 최대 길이의 제한을 알아야 하기 때문)
...
-- 컬럼 생략 가능? 가능하지만 테이블의 정의와 정확히 일치해야 함
-- 항상 컬럼을 명시하는 방식을 권장

-- 데이터 조회하기 
-- SELECT 컬럼명1, 컬럼명2, ... -- 컬럼명 대신 *을 쓰면 모든 컬럼 조회
-- FROM 테이블명
-- WHERE 조건; -- 검색 조건(생략하면 전체조회)
...
-- WHERE 조건; -- 수정 대상을 찾는 조건(생락하면 전체 대상)
...
-- 에러메세지 확인: Safe Update 모드는 워크벤치의 보호 기능으로 KEY 없는 UPDATE/DELETE 차단함

-- 임시로 안전모드 해제(권장 안함)
SET SQL_SAFE_UPDATES = 0; -- 0: 해제, 1: 설정

-- 전체 버거 조회
SELECT * FROM burgers; -- 한 줄로 써도 됨. 줄 바꿈 하는것은 가독성을 위함임
...
DELETE FROM burgers
WHERE name = '슈바두밥 버거'; -- 에러 발생: 안전모드로 인해 수정과 동일하게 조건은 반드시 기본키를 사용
