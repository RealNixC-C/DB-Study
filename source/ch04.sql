...
-- 주의
-- COUNT() 함수는 입력에 따라 다른 결과를 만듦ㅁ
-- COUNT(*): 조회된 전체 레코드의 수를 반환
...
FROM employees; -- 4 null값이 들어있는 속성은 세지 않음
...
-- 중복을 제거하여 조회하려면? DISTINCT 중복제거 키워드 적용
-- SELECT DISTINCT 컬러명
-- FROM 테이블명;
SELECT DISTINCT msg
FROM transactions;
