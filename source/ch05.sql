...
CREATE TABLE addresses (
  postal_code CHAR(5), -- 우편번호(고정 길이 문자: 5저), 주로 고정된 길이의 코드값(예: 국가코드)
  -- 문자를 3개만 넣는 경우, 자동으로 공백(space) 문자를 채움(예: 'abc  ')
  stret_address VARCHAR(100) -- 거리 주소(가변 길이 문자: 최대 100자)
...
CREATE TABLE files (
  ...
  document BLOB, -- 일반 문서 파일(최대 65KB)
  video MEDIUMBLOB, -- 비디오 파일(최대 16MB)
  large_date LONGBLOB -- 대용랑 파일(최대 4GB)
);
...
CREATE TABLE user_profiles (
  ...
  introduction TEXT,-- 자기소개(긴 문자열: 최대 64KB)
  profile_pic MEDIUMBLOB, -- 프로필 사진(파일: 최대 16MB)
...
INSERT INTO 
	user_profiles (id, email, phone, introduction, profile_pic, gender)
VALUES
	(1, 'geoblo@naver.com', '012-3456-7890', '안녕하십니까!', NULL, '남'),
	(2, 'hongsoon@example.com', '098-7654-3210', '반갑습니다요!', NULL, '여');
...
-- 사용 예
-- LIKE '케첩%': '케첩'으로 "시작"하는 임의의 문자 => 케첩 소스
-- LIKE '%케첩': '케첩'으로 "끝"나는 임의의 문자 => 토마토 케첩
-- LIKE '%케첩%': '케첩'을 "포함"하는 임의의 문자 => 토마토 케첩 소스, 케첩 소스, 토마토 케첩
...
-- MINUTE(시간): 입력 시간의 '시' 추출, 예: HOUR('2024-10-04 08:30:45') => 30
-- SECOND(시간): 입력 시간의 '시' 추출, 예: HOUR('2024-10-04 08:30:45') => 45
...
SELECT *
FROM orders
WHERE name BETWEEN 'ㄱ' AND '깋'; -- name의 첫 글자가 'ㄱ'~'깋' 사이인 row 필터랑
