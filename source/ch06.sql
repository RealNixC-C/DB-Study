-- Active: 1749830440641@@127.0.0.1@3306@stargram
/*
6. 관계 만들기
6.1 관계란
*/
...
CREATE TABLE users (
  ...
  email VARCHAR(100) UNIQUE, -- 이메일(고유한 값만 허용, 중복X)
...
CREATE TABLE capitals (
  ...
  -- 타입은 국가 테이블과 동일하게 맞춤
  -- 고유한 값만 허용(한 나라에 수도가 2개가 있으면 안됨) => UNIQUE: *일대일 관계를 만들기 위한 중요한 제약조건*
  PRIMARY KEY (id), -- 기본키 지정: id
  FOREIGN KEY (country_id) REFERENCES countries(id)
  -- countries(id) 외래키는 countries 테이블의 id 컬럼을 가르킨다는 의미(참조)
);
...
CREATE TABLE appointments (
  ...
  date DATE, -- 진료 일자
  PRIMARY KEY (id), -- 기본키 지정: id
  FOREIGN KEY (doctor_id) REFERENCES doctors(id), -- doctors테이블과 관계 형셩
  FOREIGN KEY (patient_id) REFERENCES patients(id) -- patients테이블의 관계 형성
);
...
CREATE TABLE settings (
  ...
  account_suggestions BOOLEAN NOT NULL, -- 계정 추천 여부(NULL 허용하지않음)
...
CREATE TABLE likes (
  ...
  photo_id INTEGER, -- 좋아요가 달린 사진의 아이디
  PRIMARY KEY (id), -- 기본키 지정: id
  FOREIGN KEY (user_id) REFERENCES users(id), -- 외래키 지정: user_id
  FOREIGN KEY (photo_id) REFERENCES photos(id) -- 외래키 지정: photo_id
);
