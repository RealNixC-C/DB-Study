-- Active: 1749607595528@@127.0.0.1@3306@group_analysis
/*
8. 그룹화 분석하기
8.1 그룹화란
*/
...
SELECT gender, AVG(height), COUNT(*)
FROM students
...
-- 실무에서는 AS 작명을 한글로 사용하지 않음 TOTAL_SALE 언더바와함께 사용

-- 3) SELECT 절에 올 수 있는 컬럼이 제한적이다.
-- 사용 가능한 컬럼:
-- - 그룹화 컬럼: GROUP BY 절에서 지정한 컬럼 ex) city, YEAR(sale_date);
-- - 집계된 컬럼: 집계 함수에 사용된 컬럼 MAX, MIN, AVG ...
...
-- 이렇게는 가능
SELECT SUM(id), gender, AVG(height) -- SUM 집계함수를 사용하면 가능
FROM students
GROUP BY gender;
...
SELECT 그룹화_컬럼, 집계_함수(일반_컬럼)
FROM 테이블명
WHERE 일반_필터링_조건 -- 그룹화 하기 전 개별 행(row)에 대해 필터링하는 행 단위 필터링
GROUP BY 그룹화_컬럼
HAVING 그룹_필터링_조건; -- 그룹핑된 결과에 대해 필터링하는 그룹 단위 필터링

-- 결제 유형별 평균 결제 금액이 40,000원 이상인 데이터
SELECT 
  ptype AS '결제 유형', 
...
SELECT 
  ...
  AVG(amount) AS '평균 결제 금액'
FROM payments
WHERE AVG(amount) > 40000 -- WHERE 절은 GROUP BY나 집계 함수가 처리되기 전에 실행되기 때문에
-- Invalid use of group function 오류
GROUP BY ptype
...
-- ORDER BY 절을 사용하여 수행
-- SELECT로 조회된 데이터를 기준으로 정렬하는 작업(SELECT가 먼저 수행 > ORDER BY 수행)

SELECT * -- 3)
FROM 테이블명 -- 2)
WHERE 조건 -- 1)
ORDER BY 정렬_컬럼1 ASC | DESC, 정렬_컬럼2 ASC | DESC, ...; -- 4) -- 컬럼을 여러개 지정가능
...
-- 다중 컬럼 정렬: 결제 테이블에서 결제 유형은 오름차순, 결제 금액은 내림차순
SELECT 
  ptype AS '결제 유형', 
  amount AS '결제 금액'
FROM payments
...
SELECT 
  ...
  amount AS '결제 금액'
FROM payments
ORDER BY amount DESC, ptype ASC; -- 반대로하면 그냥 결제 금액 내림차순으로됨
...
-- 결제 금액 상위 3개 데이터만 조회하라면?
SELECT * -- 2)
FROM payments -- 1)
ORDER BY amount DESC -- 3)
LIMIT 3; -- 4)

-- 상위 N개 데이터가 아닌 중간 데이터를 가져오고 싶다면?
-- LIMIT 절에 OFFSET 키워드를 추가해 읽어올 데이터의 시작 지점을 조정할 수 있음

SELECT 컬럼1, 컬럼2, ...
FROM 테이블명
LIMIT N OFFSET M; -- N: 가져올 레코드의 개수, M: 건너뛸 레코드의 개수
-- 또는 LIMIT M, N; 명시적으로 OFFSET키워드를 사용

-- 결제 금액 4등~6등까지 조회
SELECT * -- 2) 모든 컬럼을 조회
FROM payments -- 1) payments 테이블에서
ORDER BY amount DESC -- 3) 조회된 결과를 amount 기준으로 내림차순 정렬
LIMIT 3 OFFSET 3; -- 4) 처음3개는 건너뛰고(OFFSET 3) 3개의 행만 가져옴 (LIMIT 3)
...
WHERE status = '배송 완료'
GROUP BY name
ORDER BY SUM(count) DESC -- 판매수량으로 내림 차순 정렬
LIMIT 3; -- 조회를 3개로 제한함, 동률일때 출력 순서는 DB내부에서 결정하기때문에 알 수 없음
...
WHERE status = '배송 완료'
GROUP BY name
ORDER BY SUM(count) DESC, name ASC -- 정확한 출력을 위해 동률일 경우 name기준으로 오름차순 정렬
LIMIT 3;
...
SELECT -- 6) 조회
  name AS 상품명, 
  SUM(count) AS '판매 수량'
FROM orders o -- 1) 주문 테이블에서
JOIN order_details od ON o.id = od.order_id -- 2) 주문 상세 테이블을 연결
JOIN products p ON od.product_id = p.id -- 3) 상품 테이블을 연결
WHERE status = '배송 완료' -- 4) status컬럼의 '배송 완료'를 필터링
GROUP BY name -- 5) 상품명으로 그룹화
ORDER BY SUM(count) DESC, name ASC -- 7) 정렬
LIMIT 3; -- 8) 3개 제한
