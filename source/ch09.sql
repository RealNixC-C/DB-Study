-- Active: 1749607595528@@127.0.0.1@3306@market
/*
9. 서브쿼리 활용하기
9.1 서브쿼리란
*/
...
-- 3) 다양한 위치에서 사용 가능
-- SELECT
-- FROM/JOIN - 쿼리의 결과값을 테이블로 사용
-- WHERE/HAVING - 필터의 조건으로(많이쓰임)
...
SELECT 
  payment_type AS '결제 유형',
  amount AS '결제 금액',
  amount - (SELECT AVG(amount), '123' FROM payments) AS '평균 결제 금액과의 차이' -- 한개의 값만 들어올 수 있음
FROM payments;
...
-- 1) 일단 먼저 주문당 상품 개수 집계
SELECT 
  order_id, 
  SUM(count) AS total_count
FROM order_details
GROUP BY order_id; -- 전체 결과를 서브쿼리로 사용

-- 2) 메인쿼리: 1회 주문 시 평균 상품 개수 집계
SELECT AVG(total_count) -- Every derived table must have its own alias (별칭 없음 에러)
FROM (
  -- 서브쿼리
  SELECT 
  order_id, 
...
-- FROM 절에는 테이블이 와야하는데 서브쿼리의 결과는 이름이 없으므로 테이블 별칭을 부여
...
FROM (
  ...
  SELECT 
  order_id, 
  SUM(count) AS total_count -- 서브쿼리 내 집계 함수 결과에 별칭 필수(컬럼명이 아니라 계산된 값을 반환하기 때문에)
  FROM order_details
  GROUP BY order_id
) AS sub;
...
-- 상품별 주문 개수를 '배송 완료'와 '장바구니'에 상관없이 상품명과 주문 개수를 조회
-- 메인쿼리: 상품별 주문 개수
...
SELECT 
  name AS 상품명,
  total_count AS '주문 개수' -- 명시적으로 sub.total_count 작성 가능
FROM products p
JOIN (
...
-- 상품x주문상세 조인 -> 상품명으로 그룹화 -> 상품별로 매출을 집계
...
JOIN order_details od ON p.id = od.product_id
GROUP BY name
HAVING SUM(price * count) > (크림치즈의 매출):
...
-- 4. EXISTS 연산자
-- 입력받은 서브쿼리가 하나 이상의 행을반환하는 경우 TRUE, 그렇지 않으면 FALSE를 반환
