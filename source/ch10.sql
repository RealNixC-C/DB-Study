-- Active: 1749607595528@@127.0.0.1@3306@data_modeling
...
-- 예: 쇼핑몰 사이트
-- 엔티티: 사용자, 주문, 결제, 상품 (후에 Table이 될 대상들)
...
-- 관계가 필수적인지, 선택적인지를 표시
-- 필수적 관계 구현은 외래키 NOT NULL 제약 조건을 설정
...
CREATE TABLE payments (
  ...
  payments_type VARCHAR(50) NOT NULL, -- 결제 유형(필수 입력)
...
CREATE TABLE products (
  id INTEGER AUTO_INCREMENT, -- 아이디(자동으로 1씩 증가)
  name VARCHAR(50) NOT NULL UNIQUE, -- 상품명(필수입력, 고유키 지정)
  price INTEGER CHECK (price > 0), -- 가격(필수입력, 양수만 허용)
...
-- order_details 테이블 새엉
CREATE TABLE orders_details (
  id INTEGER AUTO_INCREMENT, -- 아이디(자동으로 1씩 증가)
  order_id INTEGER NOT NULL, -- 주문 아이디(필수 입력) <-- 필수적 관계
  product_id INTEGER, -- 상품 아이디(필수 입력) <-- 필수적 관계
  ...
  -- 하나의 주문에 같은 상품이 중복으로 담기지 않게 하기 위함
